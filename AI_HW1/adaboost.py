...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        # 1.Get the size of features and datasets
        # 2.create a list named error(the length of list = features length)
        # 3.for loop used to calculate error for each feature
        # 4.Select the classifier with lowest error
        # 5.Return the lowest error and the best classifier
        total_features = featureVals.shape[0]
        total_datasets = featureVals.shape[1]
        error = np.zeros(total_features)
        for j in range(total_features):
            for i in range(total_datasets):
                if featureVals[j][i] < 0:
                    error[j] += weights[i] * abs(1-labels[i])
                else:
                    error[j] += weights[i] * abs(0-labels[i])
